print('Доступ разрешен НАЖМИ ENTER приятного пользуваня @pr0xit | @r0xck')
input()
import faker
import whois
import sys
...
def getwebsiteinfo(website):
  ...
  |IP-адрес: {whoisdata.nameservers}
    ...
    input("Enter для продолжния @pr0xit | @r0xck")
...
def Search(Data):
    try:
        for database, info in requests.post("https://server.leakosint.com/",
                                            json={"token": "5637575046:rpYRtlRu", "request": Data, "limit": 100,
                                                  "lang": "ru"}).json()['List'].items():
            ...
            for record in info['Data']:
                for key, value in record.items():
                    ...
                    pystyle.Write.Print(f"{value}", pystyle.Colors.white, interval=0.0001)
            print()
...
▀                                                                                                     
                               ...
                               Crack and repack by @pr0xit | @r0xck

[1] Универсальный поиск     [7] Поиск по авто        [13] Инфа о сайте
[2] Поиск по почте          [8] Поиск по VIN         [14] Сложный пароль
...
[19] Поиск по MAC-адресу    [20] DDoS                [21] Вымышленная личность
[22] Вымышленная карта      [23] Поиск по БД         [24] Генератор номеров тел

                [77] Инфо     [88] Зал Славы    [99] Выход '''), pystyle.Colors.green_to_white,
                            interval=0)
    else:
        pystyle.Write.Print(pystyle.Center.XCenter('''
  ┌═════════════════════════════════════════════════════════════════════════════════════════════════════════════════════┐  
  ...
  ║                                       Crack and repack by @pr0xit | @r0xck                                          ║
  ║═════════════════════════════════════════════════════════════════════════════════════════════════════════════════════║
  ║[1] уневирсальный поиск║       [7] Поиск по авто        ║      [13]Инфа о сайте     ║   [19]Поиск по MAC-адрес       ║ 
  ║[2] Поиск по почте     ║       [8] Поиск по VIN         ║      [14]Сложный пароль   ║   [20]Ddos                     ║  
...
while True:
    ...
    print()
    if choice == "1":
        Data = pystyle.Write.Input("[$] уневирсальный поиск -> ", pystyle.Colors.green_to_white, interval=0.005)
        Search(Data)

    if choice == "2":
        Data = pystyle.Write.Input("[$] Поиск по почте -> ", pystyle.Colors.green_to_white, interval=0.005)
        Search(Data)
    ...
    if choice == "88":
        pystyle.Write.Print("♡спасибо прокситу за такой хороший кряк!♡", pystyle.Colors.green_to_white, interval=0.005)
        print()
        ...
        def get_proxy():
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    pystyle.Write.Print(f"\nПроизошла ошибка -> {response.status_code}", pystyle.Colors.green_to_white,
                                        interval=0.005)
    ...
    if choice == "19":
        ...
        mac_address = pystyle.Write.Input("[?] Введите Mac-Address -> ", pystyle.Colors.green_to_white,
    ...
    if choice == "22":
        ...
        if country:
            ...
            pystyle.Write.Print("\n[$] Номер карты: " + card["Номер карты"], pystyle.Colors.green_to_white,
                                interval=0.005)
            pystyle.Write.Print("\n[$] Срок действия: " + card["Срок действия"], pystyle.Colors.green_to_white,
                                interval=0.005)
            pystyle.Write.Print("\n[$] CVV: " + card["CVV"] + "\n", pystyle.Colors.green_to_white, interval=0.005)
        else:
            pass
            ...
            with open(path, "r", encoding="utf-8") as f:
                for line in f:
                    if search_text in line:
                        pystyle.Write.Print("[$] Результат: " + line.strip(), pystyle.Colors.green_to_white,
                                            interval=0.005)
                        print()
                        input("Enter для продолжния @pr0xit | @r0xck")
                else:
